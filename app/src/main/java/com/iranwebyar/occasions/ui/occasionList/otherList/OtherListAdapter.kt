...
import com.iranwebyar.occasions.databinding.ItemOtherViewBinding
...
constructor(otherItemViewModelList: List<OccasionsPOJO.Occasion?>?, callback: OtherListFragmentCallback) : BaseAdapter<OccasionsPOJO.Occasion?>() {
    var callback: OtherListFragmentCallback? = callback
    ...
    override fun onCreateViewHolderBase(parent: ViewGroup?, viewType: Int): BaseViewHolder {
        publicParent = parent
        val mOtherViewBinding = ItemOtherViewBinding
            .inflate(LayoutInflater.from(parent!!.context), parent, false)
        return OtherViewHolder(mOtherViewBinding)
    }

    inner class OtherViewHolder(var mBinding: ItemOtherViewBinding) : BaseViewHolder(mBinding.root),
        OtherItemViewModel.OtherItemViewModelListener {
        private var otherItemViewModel: OtherItemViewModel? = null
        private val handler = Handler()

        override fun onBind(position: Int) {
            val session = dataList!![position]!!
            otherItemViewModel = OtherItemViewModel(session, this)
...
//                    Color.parseColor("#FD9C95"))
    ...
    init {
        dataList = otherItemViewModelList
